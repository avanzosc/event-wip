...
class SaleOrder(models.Model):
            ...
            for line in sale_lines:
                ...
                if sale.project_by_task == 'yes':
                    if line.service_project_task:
                        ...
                        project.event_id = line.event.id
            sale.project_id.name = sale.name
    ...
    def action_cancel(self):
        for line in self.filtered(lambda x: x.project_by_task ==
                                  'yes').mapped('order_line'):
            if len(line.event_id.registration_ids) > 0:
                raise exceptions.Warning(
                    _('You can not cancel the sale order, because the event '
                      'generated by the line with sequence %s, has people '
                      'registered') % (str(line.sequence)))
            else:
                line._delete_event_information_by_task()
